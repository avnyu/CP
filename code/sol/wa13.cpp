...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        while (a[dmax.front()] - a[dmin.front()] > s && last + l - 1 < i) {
            ...
            if (dq.front() == last) dq.pop_front();
            last++;
        }
        // dbg(last, i);
        if (dq.empty() || a[dmax.front()] - a[dmin.front()] > s ||
            last + l - 1 > i)
            dp[i] = -1000000000;
        else
            dp[i] = dp[dq.front() - 1] + 1;
        while (!dq.empty() && dp[dq.front()] > dp[i]) dq.pop_back();
        dq.push_back(i);
    }
    // dbg_v(dp, n + 1);
    cout << max(dp[n], -1) << '\n';
